...
class Router implements ExecuteHooks {
	 ...
	 */
        public function init() {
                ...
                // XML Index.
                add_rewrite_rule( '^sitemaps.xml$', 'index.php?webseo_sitemap=1', 'top' );
                add_rewrite_rule( '^sitemaps.xml$', 'index.php?seopress_sitemap=1', 'top' );

                // XSL Sitemap.
                add_rewrite_rule( '^sitemaps_xsl.xsl$', 'index.php?webseo_sitemap_xsl=1', 'top' );
                add_rewrite_rule( '^sitemaps_xsl.xsl$', 'index.php?seopress_sitemap_xsl=1', 'top' );

                // XSL Video Sitemap.
                add_rewrite_rule( '^sitemaps_video_xsl.xsl$', 'index.php?webseo_sitemap_video_xsl=1', 'top' );
                add_rewrite_rule( '^sitemaps_video_xsl.xsl$', 'index.php?seopress_sitemap_video_xsl=1', 'top' );

                add_rewrite_rule( '([^/]+?)-sitemap([0-9]+)?\.xml$', 'index.php?webseo_cpt=$matches[1]&webseo_paged=$matches[2]', 'top' );
                ...
                if ( '1' === seopress_get_service( 'SitemapOption' )->authorIsEnable() ) {
                        add_rewrite_rule( 'author.xml?$', 'index.php?webseo_author=1', 'top' );
	 ...
	 */
        public function queryVars( $vars ) {
                $vars[] = 'webseo_sitemap';
                $vars[] = 'webseo_sitemap_xsl';
                $vars[] = 'webseo_sitemap_video_xsl';
                $vars[] = 'webseo_cpt';
                $vars[] = 'webseo_paged';
                $vars[] = 'webseo_author';
