...
function seopress_xml_sitemap_index_xsl() {
	...
	$seopress_sitemaps_xsl .= '<p>' . /* translators: %s number of xml sub-sitemaps */ sprintf( __( 'This XML Sitemap Index file contains %s sitemaps.', 'webseo' ), '<xsl:value-of select="count(sitemap:sitemapindex/sitemap:sitemap)"/>' ) . '</p>';
	$seopress_sitemaps_xsl .= '</xsl:if>';
	$seopress_sitemaps_xsl .= '<xsl:if test="sitemap:urlset/sitemap:url">';
	$seopress_sitemaps_xsl .= '<p>' . /* translators: %s number of URLs in an xml sitemap */ sprintf( __( 'This XML Sitemap contains %s URL(s).', 'webseo' ), '<xsl:value-of select="count(sitemap:urlset/sitemap:url)"/>' ) . '</p>';
	$seopress_sitemaps_xsl .= '</xsl:if>';
	...
	$seopress_sitemaps_xsl .= __( 'Last update', 'webseo' );
	$seopress_sitemaps_xsl .= '</div>';
