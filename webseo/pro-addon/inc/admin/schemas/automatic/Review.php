<?php

defined('ABSPATH') or exit('Please don&rsquo;t call the plugin directly. Thanks :)');

?>

<div class="wrap-rich-snippets-review">
	<div class="seopress-notice">
		<p>
			<?php /* translators: %s: link documentation */
				echo wp_kses_post(sprintf(__('Learn more about the <strong>Review schema</strong> from the <a href="%s" target="_blank">Google official documentation website</a>', 'webseo'), 'https://developers.google.com/search/docs/data-types/review-snippet'));
			?>
			<span class="dashicons dashicons-external"></span>
		</p>
	</div>
	<p>
		<label for="seopress_pro_rich_snippets_review_item_meta">
			<?php esc_html_e('Review item name', 'webseo'); ?>
			<code>item</code>
		</label>
		<?php echo seopress_schemas_mapping_array('seopress_pro_rich_snippets_review_item', 'default'); ?>
		<span class="description"><?php esc_html_e('The item name reviewed', 'webseo'); ?></span>
	</p>
	<p>
		<label for="seopress_pro_rich_snippets_review_item_type_meta">
			<?php esc_html_e('Review item type', 'webseo'); ?>
			<code>itemType</code>
		</label>
		<?php echo seopress_schemas_mapping_array('seopress_pro_rich_snippets_review_item_type', 'default'); ?>
		<span class="description"><?php echo wp_kses_post(__('<strong>Authorized values:</strong> "CreativeWorkSeason", "CreativeWorkSeries", "Episode", "Game", "MediaObject", "MusicPlaylist", "MusicRecording", "Organization"', 'webseo')); ?></span>
	</p>
	<p>
		<label for="seopress_pro_rich_snippets_review_img_meta"><?php esc_html_e('Review item image', 'webseo'); ?>
			<code>image</code>
		</label>
		<?php echo seopress_schemas_mapping_array('seopress_pro_rich_snippets_review_img', 'image'); ?>
		<span class="description"><?php esc_html_e('Review item image URL', 'webseo'); ?></span>
	</p>
	<p>
		<label for="seopress_pro_rich_snippets_review_rating_meta">
			<?php esc_html_e('Your rating', 'webseo'); ?>
			<code>ratingValue</code>
		</label>
		<?php echo seopress_schemas_mapping_array('seopress_pro_rich_snippets_review_rating', 'rating'); ?>
		<span class="description"><?php esc_html_e('Your rating: scale from 1 to 5', 'webseo'); ?></span>
	</p>
	<p>
		<label for="seopress_pro_rich_snippets_review_max_rating_meta">
			<?php esc_html_e('Max best rating', 'webseo'); ?>
			<code>bestRating</code>
		</label>
		<?php echo seopress_schemas_mapping_array('seopress_pro_rich_snippets_review_max_rating', 'rating'); ?>
		<span class="description"><?php esc_html_e('Only required if your scale is different from 1 to 5.', 'webseo'); ?></span>
	</p>
	<p>
		<label for="seopress_pro_rich_snippets_review_body_meta">
			<?php esc_html_e('Review body', 'webseo'); ?>
			<code>reviewBody</code>
		</label>
		<?php echo seopress_schemas_mapping_array('seopress_pro_rich_snippets_review_body', 'default'); ?>
		<span class="description"><?php esc_html_e('Your review body', 'webseo'); ?></span>
	</p>
</div>
