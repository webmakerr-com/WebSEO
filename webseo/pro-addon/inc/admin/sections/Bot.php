...
function seopress_print_section_info_audit() {
	...
	$last_audit = get_option('seopress_pro_site_audit_last_scan') ? wp_date( get_option( 'date_format' ) . ' - ' . get_option( 'time_format' ), get_option('seopress_pro_site_audit_last_scan') ) : esc_html('No audit performed to date.', 'webseo');
	...
	<div class="sp-section-header">
		<h2>
			<?php esc_html_e('Site Audit', 'webseo'); ?>
		</h2>
	</div>
	<p>
		<?php echo wp_kses_post(__('Find all content analysis in one place to solve technical SEO issues. Everytime you edit a post, page or post type with the <strong>published status</strong>, your site audit will be automatically updated here.', 'webseo')); ?>
	</p>
	<p>
		<?php echo /* translators: %s SEO, PRO, Google Search Console settings tab */ wp_kses_post(sprintf(__('If you have linked your <a href="%s"><strong>Google Search Console account</strong></a>, we also display <strong>clicks and positions</strong> data associated with each URL to help you prioritize your optimizations.', 'webseo'), esc_url(admin_url('admin.php?page=seopress-pro-page#tab=tab_seopress_inspect_url')))); ?>
	</p>
	<p>
		<?php echo /* translators: %s SEO, PRO, Tools, Reset settings tab */ wp_kses_post(sprintf(__('Run the Site Audit to update existing data. If you want to start from scratch, go to <a href="%s">Tools, Reset</a>.', 'webseo'), esc_url(admin_url('admin.php?page=seopress-import-export#tab=tab_seopress_tool_reset')))); ?>
	</p>
	<p>
		<?php if($running === "1") { ?>
			<button type="button" id="seopress-cancel-site-audit" class="btn btnPrimary">
				<?php esc_html_e('Cancel current audit', 'webseo'); ?>
			</button>
		<?php } else { ?>
			<button type="button" id="seopress-run-site-audit" class="btn btnPrimary">
				<?php esc_html_e('Run audit', 'webseo'); ?>
			</button>
			<?php if ($latest_offset && $latest_offset > 0) { ?>
				<button type="button" id="seopress-resume-site-audit" class="btn btnSecondary" data-offset="<?php echo esc_attr($latest_offset); ?>">
					<?php esc_html_e('Resume audit', 'webseo'); ?>
				</button>
			<?php } ?>
		<?php } ?>
	...
	<p class="last-audit">
		<?php /* translators: %s Last scan date */ printf(wp_kses_post(__('<strong>Last scan:</strong> %s', 'webseo')), esc_html($last_audit)); ?>
	</p>
	<?php if($scan_duration) { ?>
		<p>
			<?php printf(wp_kses_post(/* translators: %1$s duration in minutes, %2$s duration in seconds */ __('<strong>Scan duration:</strong> %1$s minutes %2$s seconds', 'webseo')), esc_html($scan_duration_minutes), esc_html($scan_duration_seconds)); ?>
		</p>
	<?php } ?>
	...
	<div id="seopress-notice-site-audit-running" class="seopress-notice" style="display:none">
		<p>
			<?php
				...
				/* translators: %1$d current posts analyzed, %2$d total posts */ printf(wp_kses_post(__('<span id="seopress-site-audit-offset">%1$d</span> posts analyzed out of <strong>%2$d</strong>.', 'webseo')), absint($current), absint( $count ));
			?>
		</p>
		
		<p id="seopress-site-audit-log"><?php echo esc_html($log); ?></p>
		
		<p><em><?php esc_html_e('You can close this tab, the site audit will continue to run in background.', 'webseo'); ?></em></p>
	</div>

	<hr>
	<h3>
		<?php esc_html_e('Overview', 'webseo'); ?>
	</h3>
		...
		<?php
			...
			if ($issues) { ?>
				<div class="seopress-card-item">
					<span class="title">
						<?php esc_html_e('Issues detected', 'webseo'); ?>
					</span>
					<span class="value">
						<?php
							echo esc_html($issues);
						?>
						<small><?php echo absint($hidden_issues) . ' ' . esc_html__('ignored', 'webseo'); ?></small>
					</span>
				</div>
				<div class="seopress-card-item seopress-error">
					<span class="title">
						<?php esc_html_e('High impact issues', 'webseo'); ?>
					</span>
					<span class="value">
						<?php
				...
				<div class="seopress-card-item seopress-warning">
					<span class="title">
						<?php esc_html_e('Medium impact issues', 'webseo'); ?>
					</span>
					<span class="value">
						<?php
				...
				<div class="seopress-card-item">
					<span class="title">
						<?php esc_html_e('Crawled URL', 'webseo'); ?>
					</span>
					<span class="value">
						<?php
			...
			?>
				<p>
					<?php esc_html_e('Run a scan to find issues.', 'webseo'); ?>
				</p>
			<?php
		} ?>
	</div>
...
function seopress_print_section_info_bot() {
	...
	<div class="sp-section-header">
		<h2>
			<?php esc_html_e('Scan', 'webseo'); ?>
		</h2>
	</div>
	<p><?php esc_html_e('The bot scans links in your content to find errors (404...). We limit this search by default to the last 100 posts/pages/custom post types.', 'webseo'); ?>

	<p>
		<?php esc_html_e('You can increase this value in the settings tab.', 'webseo'); ?>

		<a class="seopress-help" href="<?php echo esc_url($docs['bot']); ?>" target="_blank">
			<?php esc_html_e('Check our guide', 'webseo'); ?>
		</a>
		<span class="seopress-help dashicons dashicons-external"></span>
	</p>

	<a href="<?php echo esc_url( admin_url('edit.php?post_type=seopress_bot') ); ?>" class="btn btnTertiary">
		<?php esc_html_e('View scan results', 'webseo'); ?>
	</a>
...
{ ?>
	...
	<h3 id="seopress-bot-broken-links">
		<?php esc_attr_e('Broken links settings', 'webseo'); ?>
	</h3>
<?php
}
...
{ ?>
	...
	<div class="seopress-sub-tabs">
		<a href="#seopress-bot-site-audit"><?php esc_attr_e('Site audit settings', 'webseo'); ?></a> | 
		<a href="#seopress-bot-broken-links"><?php esc_attr_e('Broken links settings', 'webseo'); ?></a>
	</div>
	
	<hr>

	<h3 id="seopress-bot-site-audit">
		<?php esc_attr_e('Site audit settings', 'webseo'); ?>
	</h3>
<?php
}